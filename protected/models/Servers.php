...
class  Servers extends Model
{
    ...
    public function rules()
    {
        $rules = [
            [['host'], 'required'],
            [['status', 'weight', 'last_call_id'], 'integer', 'integerOnly' => true],
            [['host', 'name'], 'string', 'max' => 100],
            [['description'], 'string', 'max' => 500],
            [['password', 'username', 'last_call'], 'string', 'max' => 50],
            [['type', 'port', 'sip_port'], 'string', 'max' => 20],
            [['public_ip'], 'string', 'max' => 80],
    ...
    public function checkpassword($attribute)
    {
        ...
        $modelUser = User::find()->where(['password' => $this->password])->one();
