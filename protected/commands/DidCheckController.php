...
class DidCheckController extends ConsoleCommand
{
    public function actionRun($args = '')
    {
        $query = DidUse::find();
        $query->where(['and', ['or', ['releasedate' => null], ['<', 'releasedate', '1984-01-01 00:00:00']], ['status' => 1]]);
        $query->joinWith(['idDid' => function ($query) {
            $query->andwhere(['and', ['<>', 'pkg_did.billingtype', 3], ['>', 'pkg_did.fixrate', 0]]);
        }]);
        $modelDidUse = $query->all();
            ...
            if ($day_remaining >= 0) {
                ...
                if ($day_remaining <= (intval($daytopay) * $oneday)) {
                    ...
                    if ($user_credit >= $didUse->idDid->fixrate) {
                        ...
                        $modelDid = Did::findOne((int) $didUse->id_did);
                        $modelDid->reserved = 0;
                        $modelDid->id_user  = null;
                        $modelDid->save();

                        Diddestination::deleteAll(['id_did' => $didUse->id_did]);
