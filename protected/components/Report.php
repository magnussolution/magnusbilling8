...
class Report extends FPDF
{
    ...
    public function createNameFile($str)
    {
        ...
        return strtolower(str_replace(' ', '', str_replace($search, $replace, $str)));
    }

    public function Header()
    {
        if ($this->PageNo() === 1) {
            $date = mb_convert_encoding($this->strDate, 'ISO-8859-1', 'UTF-8') . date($this->formatDate);

            if (isset($_SESSION[$this->idxUserSession])) {
                if (session_status() == PHP_SESSION_NONE) {
                    session_start();
                }
                $user = mb_convert_encoding($this->strUser, 'ISO-8859-1', 'UTF-8') . $_SESSION[$this->idxUserSession];
            ...
            $this->Cell(0, 6, mb_convert_encoding($this->title, 'ISO-8859-1', 'UTF-8'), 0, 0, 'C');
            $this->Ln(5);

            if ($this->subTitle) {
                $this->SetFont($this->fontFamily, 'B', $this->fontSize);
                $this->Cell(0, 6, mb_convert_encoding($this->subTitle, 'ISO-8859-1', 'UTF-8'), 0, 0, 'C');
            }

            if ($this->subTitle2) {
                $this->Ln(5);
                $this->SetFont($this->fontFamily, 'B', $this->fontSize);
                $this->Cell(0, 6, mb_convert_encoding($this->subTitle2, 'ISO-8859-1', 'UTF-8'), 0, 0, 'C');
            }
            if ($this->subTitle3) {
                $this->Ln(5);
                $this->SetFont($this->fontFamily, 'B', $this->fontSize);
                $this->Cell(0, 6, mb_convert_encoding($this->subTitle3, 'ISO-8859-1', 'UTF-8'), 0, 0, 'C');
            }
            if (isset($this->detailstitle)) {
                $this->Ln(15);
                $this->SetFont($this->fontFamily, 'B', $this->fontSize);
                $this->Cell(0, 5, mb_convert_encoding($this->detailstitle, 'ISO-8859-1', 'UTF-8'), 0, 0, 'C');
            }
            ...
            if (isset($this->details)) {
                $this->Ln(35);
                $this->MultiCell(0, 5, mb_convert_encoding($this->details, 'ISO-8859-1', 'UTF-8'), 1);
                $this->Ln(15);
            } else {
                $this->Ln(20);
            }
    ...
    public function bodyExtra()
    {
        ...
        if (isset($this->secondListTitle)) {
            $this->Ln(15);
            $this->SetFont($this->fontFamily, 'B', $this->fontSize);
            $this->Cell(0, 5, mb_convert_encoding($this->secondListTitle, 'ISO-8859-1', 'UTF-8'), 0, 0, 'C');
            $this->Ln(10);
        }
        $this->SetFont($this->fontFamily, 'B', $this->fontSize);
        ...
        foreach ($this->columnsDetails as $column) {
            $header = mb_convert_encoding($column['header'], 'ISO-8859-1', 'UTF-8');
            if ($column['dataIndex'] === $this->fieldGroup) {
                $headerGroup = mb_convert_encoding($column['header'], 'ISO-8859-1', 'UTF-8');
                continue;
            }
        ...
        foreach ($this->recordsDetails as $row) {
            ...
            foreach ($row as $fieldName => $col) {
                if ($fieldName == $this->fieldGroup) {
                    ...
                    $lastValue    = mb_convert_encoding($this->recordsDetails[$idxLastValue][$fieldName], 'ISO-8859-1', 'UTF-8');
                    $value        = mb_convert_encoding($col, 'ISO-8859-1', 'UTF-8');
                    ...
                    if (is_array($this->fieldsFk) && array_key_exists($fieldName, $this->fieldsFk)) {
                        ...
                        $value    = mb_convert_encoding($renderer, 'ISO-8859-1', 'UTF-8');
    ...
    public function body()
    {
        ...
        if (isset($this->firstListTitle)) {
            $this->Ln(15);
            $this->SetFont($this->fontFamily, 'B', $this->fontSize);
            $this->Cell(0, 5, mb_convert_encoding($this->firstListTitle, 'ISO-8859-1', 'UTF-8'), 0, 0, 'C');
            $this->Ln(10);
        }
        ...
        foreach ($this->records as $row) {
            ...
            foreach ($row as $fieldName => $col) {
                if ($fieldName == $this->fieldGroup) {
                    $idxLastValue = $rowNumber === 0 ? $rowNumber : $rowNumber - 1;
                    $lastValue    = mb_convert_encoding($this->records[$idxLastValue][$fieldName], 'ISO-8859-1', 'UTF-8');
                    $value        = mb_convert_encoding($col, 'ISO-8859-1', 'UTF-8');
                    if ($lastValue == $value && $rowNumber != 0) {
                        continue;
                    }
    ...
    {
        if ($value == 0) {
            return mb_convert_encoding(Yii::t('zii', 'Standard'), 'ISO-8859-1', 'UTF-8');
        } else if ($value == 1) {
            return Yii::t('zii', 'SIP');
        } else if ($value == 2) {
    ...
    public function formatBoolean($value)
    {
        if ($value) {
            $this->SetTextColor(0, 128, 0);
            return mb_convert_encoding($this->yesValue, 'ISO-8859-1', 'UTF-8');
        } else {
            $this->SetTextColor(255, 0, 0);
            return mb_convert_encoding($this->noValue, 'ISO-8859-1', 'UTF-8');
        }
    }
    ...
    public function Footer()
    {
        $this->SetY(-15);
        $this->SetFont($this->fontFamily, '', $this->fontSize);
        $this->Cell(0, 10, mb_convert_encoding($this->strPage, 'ISO-8859-1', 'UTF-8') . $this->PageNo() . mb_convert_encoding($this->strOf, 'ISO-8859-1', 'UTF-8') . '{nb}', 0, 0, 'C');
    }
}
