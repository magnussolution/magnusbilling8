...
class Util
{
    ...
    public static function getNewUsername($required = true)
    {
        ...
        if ($generate_username == 1) {
            ...
            if (isset($_SESSION['id_group']) && Yii::$app->session['id_group'] > 0) {
                $modeGroupUser = GroupUser::find()
                    ->where(['id' => Yii::$app->session['id_group']])
                    ->one();
            }
            ...
            while ($existsUsername) {
                ...
                $countUsername  = User::find()->where(['like', 'username', $randUserName])->count();
    ...
    public static function getNewSip()
    {
        ...
        while ($existsUsername) {
            $randUserName  = mt_rand(10000, 99999);
            $countUsername = Sip::find()->where(['like', 'name', $randUserName])->count();
    ...
    public static function getNewLock_pin()
    {
        ...
        while ($existsLock_pin) {
            ...
            $countLock_pin = Signup::find()->where(['like', 'callingcard_pin', $randLock_Pin])->count();
