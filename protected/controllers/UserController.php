...
class UserController extends CController
{
    ...
    public function beforeSave($values)
    {
        ...
        if ($this->isNewRecord) {
            ...
            if (Yii::$app->session['user_type'] == 2) {
                ...
                $modelAgent = User::findOne((int) $values['id_user']);
    ...
    public function afterSave($model, $values)
    {
        ...
        if (! $this->isNewRecord && isset($model->id_group_agent) && $model->id_group_agent > 1) {
            $modelUser = User::findOne(['id_user' => $model->id]);
        ...
        if (Yii::$app->session['isAdmin']) {
            ...
            if ($this->isNewRecord) {
                ...
                if (isset($values['id'])) {
                    ...
                    $modelUser = User::findOne($values[0]['id']);
                }
            ...
            if (isset($user_id_group)) {
                ...
                if (isset($modelGroupUser->id)) {
                    ...
                    $modelModels = Module::find()->where(['module' => 'groupuser'])->one();

                    $modelGroupModule = GroupModule::find()->where([
                        'id_group' => Yii::$app->session['id_group'],
                        'id_module' => $modelModels->id,
                    ])->one();
    ...
    public function actionCredit()
    {
        if (! Yii::$app->session['id_user']) {
            $module = 'User'; // Define the module name
    ...
    public function actionBulk()
    {
        ...
        $modelGroupUser = GroupUser::findOne((int) $values['id_group']);
    ...
    public function actionResendActivationEmail()
    {
        ...
        $modelUser = User::findOne((int) $_POST['id']);
    ...
    public function setAttributesModels($attributes, $models)
    {
        ...
        for ($i = 0; $i < count($pkCount); $i++) {
            ...
            if ($attributes[$i]['id_offer'] > 0) {
                ...
                $modelOffer     = Offer::findOne($attributes[$i]['id_offer']);
                ...
                $modelSip = Sip::find()->where(['id_user' => $attributes[$i]['id']])->count();
    ...
    public function freeCallUsed($id_user, $id_offer, $billingtype, $startday)
    {
        ...
        $modelOfferCdr = OfferCdr::findBySql($sql)->one();
