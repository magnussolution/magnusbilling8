...
class BaseController extends Controller
{
    ...
    public function init()
    {
        ...
        $this->modelName  = isset($this->abstractModel) ? substr(strrchr(get_class($this->instanceModel), "\\"), 1) : '';
    ...
    {
        if (preg_match('/pkg_cdr/', $this->instanceModel::tableName()) && $this->config['global']['remove_count_cdr'] == 1) {
            $sql    = "SHOW TABLE STATUS LIKE  '" . $this->instanceModel::tableName() . "'";
            ...
            $recordCont = $command->queryAll();


            return isset($recordCont[$this->instanceModel->primaryKey()[0]]) ? $recordCont[$this->instanceModel->primaryKey()[0]] : [];
        }
    }
    /**
    ...
    public function actionSave()
    {
        ...
        if (Yii::$app->session['isClient'] && $this->instanceModel::tableName() != 'pkg_user') {
        ...
        $model = $id > 0 ? $this->loadModel($id, $this->abstractModel) : $this->instanceModel;
        ...
        try {
            ...
            foreach ($values as $fieldName => $value) {
                if (isset($value['isPercent']) && is_bool($value['isPercent'])) {
                    ...
                    if ($value['isAdd']) {
                        $valueUpdate = new Expression("$fieldName + $valuePercent");
                        $valueUpdate = new Expression("$fieldName + $valuePercent");
                    } else if ($value['isRemove']) {
                        $valueUpdate = new Expression("$fieldName - $valuePercent");
                    } else {
                        $valueUpdate = new Expression($valuePercent);
                    }
                } else {
            ...
            $sql = "UPDATE " . $this->instanceModel::tableName() . " SET " . implode(', ', array_map(function ($v, $k) {
                return sprintf("%s = '%s'", $k, $v);
            }, $setters, array_keys($setters))) . " WHERE $namePk IN(" . implode(',', $ids) . ")";
            try {
    ...
    public function actionReport()
    {
        ...
        $report->records        = $this->instanceModel::tableName() == 'pkg_user' ? $records : $this->getAttributesModels($records);
        $report->generate();
    }
    ...
    public function actionCsv()
    {
        ...
        if (! isset(Yii::$app->session['id_user'])) {
            ...
            $info = 'User try export CSV ' . $this->instanceModel::tableName();
            MagnusLog::insertLOG(7, $info);
        }
    ...
    public function actionDestroy()
    {
        ...
        $values       = $this->getAttributesRequest();
        $namePk       = $this->instanceModel::primaryKey()[0];
        $arrayPkAlias = explode('.', $this->instanceModel::primaryKey()[0]);
        $ids          = [];
        ...
        if ((isset($_POST['filter']) && strlen($_POST['filter']) > 0)) {
            ...
            if (count($this->relationFilter)) {
                ...
                # retorna o resultado da execucao
                try {
                    $sql = 'DELETE FROM ' . $this->instanceModel::tableName() . ' WHERE ' . $namePk . ' IN (' . implode(',', $ids) . ')';
    ...
    public function loadModel($id, $model)
    {
        ...
        if (is_array($id)) {
            ...
            $resultModel = $this->abstractModel->where($condition)->all();
            $resultModel = array_key_exists(0, $resultModel) ? $resultModel[0] : null;
        } else {
            $resultModel = $this->abstractModel->where('id = :key')->addParams([':key' => $id])->one();
            if ($resultModel === null) {
            ...
            if ($this->actionName != 'destroy' && ! preg_match("/^id[A-Z]/", $field)) {
                ...
                if (is_array($field)) {
                    foreach ($field as $key => $fieldOr) {
                        $field[$key] = strpos($fieldOr, '.') === false ?  $fieldOr : $fieldOr;
                    }
                } else {
                    $field = strpos($field, '#') === 0 ? str_replace('#', '', $field) : (strpos($field, '.') === false ?  $field : $field);
                }
            }

            $value     = isset($f->value) ? $f->value : new Expression('NULL');
            $paramName = "p$key";
    ...
    {
        $command = Yii::$app->db->createCommand('SHOW COLUMNS FROM ' . $this->instanceModel::tableName());
    ...
    public function getColumnsFromReport($columns, $fieldGroup = null, $type = 'csv')
    {
        ...
        foreach ($columns as $column) {
            ...
            if (is_array($this->fieldsFkReport) && array_key_exists($fieldName, $this->fieldsFkReport)) {
                ...
                if (($fieldName == 'id' && $fieldReport == 'destination') ||
    ...
    {
        if ($this->instanceModel::tableName() == 'pkg_user') {
