...
class PlanCheckController extends ConsoleCommand
{
    public function actionRun($args = '')
    {
        ...
        $modelOfferUse = OfferUse::find()
            ->where(['and', ['status' => 1], ['or', ['releasedate' => null], ['<', 'releasedate', '1984-01-01 00:00:00']]])
            ->all();
        ...
        foreach ($modelOfferUse as $offerUse) {
            ...
            $timestamp_datetopay       = mktime(
                ...
                date("m", $diff_reservation_daytopay) + $offerUse->month_payed,
            ...
            if ($day_remaining >= 0) {
                ...
                if ($day_remaining <= (intval($daytopay) * $oneday)) {
                    ...
                    if ($user_credit >= $offerUse->idOffer->price) {
                        ...
                        $offerUse->idUser->updateAttributes([
                    ...
                    if ($user_credit >= $offerUse->idOffer->price) {
                        ...
                        $offerUse->status      = 0;
                        $offerUse->save();
                        $offerUse->idUser->updateAttributes(['id_offer' => 0]);
