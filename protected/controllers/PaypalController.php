...
class PaypalController extends CController
{
    ...
    public function actionIndex()
    {
        ...
        if ($verified) {
            Yii::error('EPAYMENT PAYPAL: verification successful', 'info');

            $modelMethodpay = Methodpay::find()->where(['like', 'payment_method', 'Paypal'])->one();
            ...
            Yii::error('EPAYMENT PAYPAL: OK CONTINE TO ADD CREDIT', 'info');

            if ($_POST['payment_status'] == 'Completed') {
                Yii::error('PAYMENT VERIFIED', 'info');
                $modelUser = User::findOne((int) $id_user);

                if (isset($modelUser->id) && Refill::countRefill($txn_id, $modelUser->id) == 0) {
                    ...
                    if ($this->config['global']['paypal_new_user'] == 0) {
                        ...
                        if ($modelRefillCount == 0) {
                            ...
                            Yii::error($modelUser->id . ' ' . $amount . ' ' . $description . ' ' . $txn_id, 'error');
                    ...
                    Yii::error('USERNAE NOT FOUND', 'info');
                }
            }
        } else {
            Yii::error('EPAYMENT PAYPAL: VERIFICATION FAILED', 'info');
        }
        header("HTTP/1.1 200 OK");
    }
}
...
class PaypalIPN
{
    ...
    {
        if (!count($_POST)) {
            Yii::error('EPAYMENT PAYPAL: Missing POST Data', 'error');
        ...
        Yii::error(print_r($res, true), 'error');
        if (!($res)) {
            $errno  = curl_errno($ch);
            $errstr = curl_error($ch);
            curl_close($ch);
            Yii::error("cURL error: [$errno] $errstr", 'error');
        ...
        if ($http_code != 200) {
            Yii::error("PayPal responded with http code $http_code", 'error');
